...
grantedPatents <- list() # initializing new column where amount of patents will be kept
companies <- list()
...
for(i in 1:length(tab3$Company.name)){
  ...
  for (j in length(companies[[i]])){
    numOfGranted <- (length(agrep(companies[[i]][j], 
                                  droplevels(tab4$Applicant[tab4$Status=="Granted"]))))
    
    if (numOfGranted != 0){grantedPatents[i] <- numOfGranted} 
    else{grantedPatents[i] <- 0}
  }
  }
